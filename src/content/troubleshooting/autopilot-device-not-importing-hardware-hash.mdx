...
If the device imports and receives a profile but later stops syncing after enrolment, use [Intune Device Not Syncing](/troubleshooting/intune-device-not-syncing). For fleet reporting after enrolment, the [Microsoft Graph PowerShell migration](/tutorials/azuread-msonline-to-microsoft-graph-powershell-migration) script gives a useful audit pattern.
...
<ScreenshotSlot
  src="/images/evidence/intune-autopilot-devices.png"
  alt="Microsoft Intune admin centre, Devices, Enrollment, Windows Autopilot devices list, with serial numbers and tenant identity redacted"
  caption="Windows Autopilot devices blade, including Import. Serial numbers and tenant identity redacted. Profile status still visible."
/>
...
If you are preparing a Windows 11 deployment flow, compare your profile design with [Windows Autopilot v1 vs v2](/comparisons/autopilot-v1-vs-v2-2026).
...
- Build a monthly stale-device review using the [Microsoft Graph PowerShell migration](/tutorials/azuread-msonline-to-microsoft-graph-powershell-migration) script and your normal asset inventory.

## Home lab notebook (one VM hash, one induced CSV miss)

Collect the hash on a home Hyper-V or physical spare PC. Never publish a work-device hash. This revision does not claim the notebook was executed.

1. On the lab VM, collect the hardware hash with `Get-WindowsAutopilotInfo` from the current [PowerShell Gallery script](https://www.powershellgallery.com/packages/Get-WindowsAutoPilotInfo). Use `-OutputFile` to a one-row CSV.
2. Compare that file to the starter headers in the [Autopilot hardware-hash template](/templates). Replace every `REPLACE-` value. Do not invent a hash.
3. Import the one-row CSV in **Intune > Devices > Windows > Windows enrollment > Devices**. Record the serial as a lab label if you do not want the raw serial in notes.
4. Assign a user-driven Entra join profile to a group that contains only that Autopilot device. Wait for profile assignment status, not just import success.
5. Induce one miss: upload a CSV with a truncated hash or a duplicate serial, capture the portal error, then import the good row.
6. When finished, delete the lab device from Autopilot devices **and** the Entra device object if you created one. Do not leave a home VM registered in a tenant you later reuse for work.

<EvidenceCallout
  type="screenshot"
  title="Autopilot import result for the lab VM"
  capture="Intune Windows Autopilot devices row showing serial, manufacturer, profile status, and import time"
  redact="Serial number if it identifies a work device, tenant name, object IDs, and the hardware hash"
  why="Registration is not enrolment. Profile status is the proof this notebook is about, not a later Company Portal sign-in."
/>

Do not set `labTested` until the good row assigns a profile and the bad row has a named error. Do not attach the `.csv` with a real hash to this repository.
...
- [Windows Autopilot v1 vs v2](/comparisons/autopilot-v1-vs-v2-2026)
- [Microsoft Graph PowerShell migration](/tutorials/azuread-msonline-to-microsoft-graph-powershell-migration)
- [Windows Autopilot registration overview](https://learn.microsoft.com/en-us/autopilot/registration-overview)
- [Manually register devices with Windows Autopilot](https://learn.microsoft.com/en-us/autopilot/add-devices)
- [Windows Autopilot profiles](https://learn.microsoft.com/en-us/autopilot/profiles)

**Verification clarification, 19 September 2026:** Added a one-device home-lab notebook. No hash collection or import is claimed.
