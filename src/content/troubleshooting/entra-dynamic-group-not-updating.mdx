...
4. Check supported include/exclude assignments for the workload. For apps, exclusions take precedence for the same group type; do not expect a device exclusion to cancel a user inclusion. See [Microsoft assignment rules](https://learn.microsoft.com/en-us/intune/app-management/deployment/configure-assignment-scope).
...
For reporting and cleanup patterns, see [Microsoft Graph PowerShell migration](/tutorials/azuread-msonline-to-microsoft-graph-powershell-migration).
...
**Correction — 10 September 2026:** Replaced a nonexistent exclusion priority rule with workload-specific include/exclude behaviour.
