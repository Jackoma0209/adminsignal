...
labTested: "26 September 2026"
labScope: "home-lab"
reviewNote: "Home-lab HealthScripts report for three policies. Results were sent. Portal status names were not captured, and this is not a full procedure test."
...
For fleet work, export the status before editing the package. You can also build your own repeatable export using [Microsoft Graph PowerShell migration](/tutorials/azuread-msonline-to-microsoft-graph-powershell-migration).
...
The following marker example is for an isolated lab device only. It writes a new AdminSignal test value and is not a complete production repair tool. Confirm the test key was absent or back up its existing value before testing; remove only the marker created by this test when finished. Use this minimal pattern when testing:
...
try {
    ...
    Write-Output "Detection could not read the value; investigate before repair."
    exit 2
}
```
...
try {
    ...
    New-ItemProperty -Path $Path -Name "Remediated" -Value "Yes" -PropertyType String -Force -ErrorAction Stop | Out-Null
    Write-Output "Set remediation marker; rerun detection to validate."
    exit 0
} catch {
    Write-Output "Marker write failed: $($_.Exception.Message)"
    exit 1
}
```
...
Keep detection read-only, including in a detection-only package. A failed read is not proof that a repair is needed. Only exit 1 should request remediation; see [Microsoft’s Remediations contract](https://learn.microsoft.com/en-us/intune/device-management/tools/deploy-remediations).
...
$Device = Get-MgDevice -DeviceId "<verified-entra-object-id>" -ErrorAction Stop
...
HealthScripts on one home-lab device started a report, sent it, and recorded that the reports were sent. The payload had three policies. This is a summary, not the JSON blob, and it omits account, policy and user identifiers. Portal status names were not captured. The next runner was queued for UTC 27/09/2026 00:00:00. Agent version 1.105.152.0, OS 10.0.26200.

Redacted excerpt:

```text
<![LOG[[HS] starting to send report]LOG]!><time="15:26:44.8158057" date="9-26-2026" component="HealthScripts" context="" type="1" thread="50" file="">
<![LOG[[HS] sending results to service..]LOG]!><time="15:28:42.9048681" date="9-26-2026" component="HealthScripts" context="" type="1" thread="75" file="">
<![LOG[[HS] Reports are sent successfully, mark them as sent in local cache.]LOG]!><time="15:28:43.5816892" date="9-26-2026" component="HealthScripts" context="" type="1" thread="75" file="">
```

| Detect output | Remediate output | Post-detect | Exit codes | What it shows |
|---|---|---|---|---|
| WARNING: App Compat not run in last 24 hours | App Compat Assessment started | same warning | detect 1 / remediate 1 / last detect 1 | Detect ran, remediate ran, issue remained |
| Windows Store Apps updated. | none | none | first detect 0 | Detect healthy, no remediate |
| Windows Telemetry registry settings are incorrect | Windows Telemetry registry settings are correct | correct | first detect 1 / remediate 0 / last detect 0 | Full detect → remediate → healthy trio |
...
```powershell
$Script = "C:\Temp\Detect-Example.ps1"
$Duration = Measure-Command { & powershell.exe -NoProfile -File $Script }
...
If local logs show the script ran successfully but the portal has not changed, wait for reporting or export the script output. For reporting and stale device lists, use the Graph patterns in [Microsoft Graph PowerShell migration](/tutorials/azuread-msonline-to-microsoft-graph-powershell-migration).
...
## Home lab notebook (harmless registry probe)

Use the detect and remediate scripts in this repository under `scripts/lab`. They only write `HKLM:\SOFTWARE\AdminSignalLab\Probe`. Do not point a remediation at a security control you cannot roll back. The recorded HealthScripts report earlier on this page is the three-policy summary, not output from these probe scripts.

1. Confirm Intune Remediations licensing on the lab user.
2. Create a remediation with 64-bit PowerShell, assigned to the lab VM only, run once or on a short schedule you can wait for.
3. Detect script: `Detect-AdminSignalLabProbe.ps1` (exit 1 when the value is missing).
4. Remediate script: `Remediate-AdminSignalLabProbe.ps1` (creates `Probe=healthy`).
5. Capture three Intune statuses: issue detected, remediated, then healthy on the next detect. Also capture `HealthScripts.log` lines for that run.
6. Roll back by deleting `HKLM:\SOFTWARE\AdminSignalLab` and removing the assignment.

Microsoft's [Remediations](https://learn.microsoft.com/en-us/intune/device-management/tools/deploy-remediations) document is the product contract; this notebook is the one-script proof.

<EvidenceCallout
  type="output"
  title="Remediation detect then remediate on the lab VM"
  capture="Intune device status for the probe script across detect-unhealthy, remediated, and detect-healthy, plus HealthScripts.log timestamps"
  redact="Device name, UPN, and tenant"
  why="A remediation that never leaves Pending is not evidence. Three statuses on one device are."
/>

The 26 September 2026 home-lab label covers that HealthScripts report only. It does not include the three portal statuses named above.
...
**26 September 2026 — Home-lab:** A redacted HealthScripts report shows the send started, the results went to the service, and the reports were marked sent. Three policies are summarised in Step 6. That capture does not verify the rest of this procedure. Other examples remain illustrative.

## Correction record

**26 September 2026 — Home-lab:** Added the redacted HealthScripts send lines and the three-policy summary. Portal status names were not captured.

**19 September 2026:** Added a one-device home-lab notebook using the AdminSignalLab registry probe scripts. No tenant execution is claimed.

**10 September 2026 — Reviewed against documentation:** Made the illustrative marker fail on write errors, separated failed reads from missing values, and used an isolated PowerShell process and verified device identity. The code samples from that pass remain illustrative.
