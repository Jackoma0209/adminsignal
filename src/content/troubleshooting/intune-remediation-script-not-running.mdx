...
Use the detect and remediate scripts in this repository under `scripts/lab`. They only write `HKLM:\SOFTWARE\AdminSignalLab\Probe`. Do not point a remediation at a security control you cannot roll back. This revision does not claim the notebook was executed.
...
Do not set `labTested` until those three statuses exist.
...
**10 September 2026 — Reviewed against documentation:** Made the illustrative marker fail on write errors, separated failed reads from missing values, and used an isolated PowerShell process and verified device identity. Code and output remain illustrative, not execution evidence.
