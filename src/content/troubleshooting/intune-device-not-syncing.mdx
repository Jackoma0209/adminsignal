...
reviewNote: "Corrected dsregcmd MDM URL interpretation and replaced unsupported whole-procedure testing claims with scope and assumptions."
...
This is documentation-based troubleshooting guidance, not a documented end-to-end lab test. The existing redacted portal capture illustrates one check-in screen only. Use a supported Windows release, or a Windows 10 device covered by applicable servicing rights, enrolled in Microsoft Intune. It covers Microsoft Entra joined, Microsoft Entra hybrid joined, and co-managed Windows devices where Intune is expected to deliver MDM policy.
...
| `dsregcmd /status` has no MDM URL | Check tenant MDM configuration and current-user enrolment scope; corroborate with device evidence |
...
For reporting across multiple devices, the [Microsoft Graph PowerShell migration](/tutorials/azuread-msonline-to-microsoft-graph-powershell-migration) script is a useful companion pattern.

## Step 2: Check the Intune Portal State

<ScreenshotSlot
  src="/images/evidence/intune-device-overview-checkin.png"
  alt="Microsoft Intune admin centre Devices > Windows devices > device Overview, showing Last check-in 23/08/2026 03:13 pm with Compliant, Corporate, and Co-managed badges. Device name, serial, user, and tenant redacted"
  caption="Intune device Overview last check-in. This capture is current (23/08/2026, 03:13 pm), not a stale-sync example. Device name, serial, user, and tenant redacted."
/>
...
| `MdmUrl` | Reflects tenant MDM configuration and user enrolment scope; presence does not prove device enrolment |
...
| Enrolment keys exist but no `MdmUrl` in `dsregcmd` | Check tenant MDM configuration and the current user’s enrolment scope; this alone does not establish broken enrolment |
...
$ManagedDevices = Get-GraphCollectionResponse -Uri $Uri
...
| `MdmUrl` missing | Review tenant MDM configuration and user scope; do not trigger re-enrolment from this field alone |
...
**10 September 2026 — Reviewed against documentation:** Corrected dsregcmd MDM URL interpretation and replaced unsupported whole-procedure testing claims with scope and assumptions. Code and output remain illustrative, not execution evidence.
