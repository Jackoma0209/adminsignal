...
<EvidenceCallout
  type="output"
  title="dsregcmd /status join and MDM fields"
  image="/images/evidence/cmd-dsregcmd.png"
  imageAlt="Redacted dsregcmd /status output showing AzureAdJoined YES, DeviceAuthStatus SUCCESS, MdmUrl present, and AzureAdPrt YES, with tenant identifiers masked"
  capture="Command Prompt output of dsregcmd /status showing AzureAdJoined, DomainJoined, DeviceAuthStatus, TenantId, MdmUrl, and AzureAdPrt"
  redact="Tenant display name, TenantId, device IDs, and any user or workplace-join identifiers"
  why="A stale Last check-in in Intune is not proof that MDM is still enrolled. Missing MdmUrl or a failed DeviceAuthStatus means the Sync button cannot repair the device."
/>
