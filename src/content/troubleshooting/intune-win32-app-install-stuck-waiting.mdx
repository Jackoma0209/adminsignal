...
reviewNote: "Partial IME capture. IME is running: TLS pin Success and Proxy Poller Failed 1008 / 0x80004005. Not a Win32 install or detection transition. Documentation-reviewed; the waiting, pending and failed install path was not lab-tested."
...
For the wider endpoint management context, see the [Intune hub](/intune). For reusable scripts and report exports, see the [PowerShell hub](/powershell) and [Microsoft Graph PowerShell migration](/tutorials/azuread-msonline-to-microsoft-graph-powershell-migration).
...
<ScreenshotSlot
  src="/images/evidence/intune-win32-device-install-status.png"
  alt="Microsoft Intune admin centre, Apps, Windows, Device install status, with device names and UPNs redacted"
  caption="Win32 app Device install status blade. Device names and UPNs redacted. This capture shows Installed, not a stuck Waiting for install row."
/>
...
Partial IME capture. This is IME health and proxy activity, not AppWorkload. It shows the extension running: TLS pin Success, then Proxy Poller Failed 1008 / 0x80004005. It is not a Win32 install or detection transition. The waiting, pending and failed install path was not lab-tested.

Redacted excerpt:

```text
<![LOG[Managed TLS pinning completed. Scenario=SidecarGateway, Hostname=LabPC, Success=True, Failure=None, HResult=0x00000000, SslPolicyErrors=None, Status=Trusted, IsPinned=True, StaticPinEvaluationEnabled=True, StaticHostnameMatched=True, StaticPinTrusted=True, UsedStaticPinFallback=False, ErrorFlags=0x0000000000000000, DurationMs=3.]LOG]!><time="12:48:08.5567620" date="9-26-2026" component="IntuneManagementExtension" context="" type="1" thread="3" file="">
<![LOG[[Proxy Poller] LogonUser failed with error code : 1008]LOG]!><time="14:36:20.4166534" date="9-26-2026" component="IntuneManagementExtension" context="" type="3" thread="83" file="">
<![LOG[[Proxy Poller] Failed to get proxy info for session id 1, exception is System.ComponentModel.Win32Exception (0x80004005): An attempt was made to reference a token that does not exist
<![LOG[Successfully updated throttling info. workload AgentCheckIn, currentCnt = 1]LOG]!><time="15:20:07.8931963" date="9-26-2026" component="IntuneManagementExtension" context="" type="1" thread="64" file="">
```
...
- [ ] Intune app reporting is exported after pilot deployment. See [Microsoft Graph PowerShell migration](/tutorials/azuread-msonline-to-microsoft-graph-powershell-migration).

## Home lab notebook (one freeware Win32 app)

Package 7-Zip or Notepad++ only if the licence allows redistribution. Do not wrap paid software. This revision does not claim the notebook was executed.

1. Download the vendor installer to the lab PC, not a work device.
2. Wrap it with the Microsoft Win32 Content Prep Tool. Keep install and uninstall command lines that you actually tested locally (`msiexec` or the silent EXE switch from the vendor).
3. In Intune, add a Windows app (Win32) assigned as **Required** to the lab VM only.
4. Detection: a file exists rule for the real binary path, for example `C:\Program Files\7-Zip\7z.exe`. Do not detect on a directory that already exists.
5. On the VM, wait for IME. Capture `IntuneManagementExtension.log` and `AppWorkload.log` lines for the app's GUID when status is Success.
6. Induce a miss: change detection to a path that does not exist, sync, capture Failed, then restore the good rule.

Microsoft's [Win32 app troubleshooting](https://learn.microsoft.com/en-us/troubleshoot/mem/intune/app-management/troubleshoot-win32-app-install) is the log map; this notebook is the one-app proof.

<EvidenceCallout
  type="output"
  title="IME install status for the lab Win32 app"
  capture="Intune device install status Success, then Failed after the broken detection rule, plus the IME log snippet for that app GUID"
  redact="Device name, tenant, app GUID if it identifies a production package, and user UPN"
  why="Waiting is not a diagnosis. Success then a deliberate Failed detection is the smallest original evidence this page can grow from."
/>

Do not set `labTested` until both statuses exist. Do not commit the `.intunewin` file.
...
**26 September 2026 — Partial IME capture:** A redacted `IntuneManagementExtension.log` slice shows IME health and proxy activity, including TLS pin Success and Proxy Poller Failed 1008 / 0x80004005. This page stays documentation-reviewed. The slice is not a Win32 install or detection transition, and the waiting, pending and failed install path was not lab-tested. Other examples remain illustrative.

## Correction record

**26 September 2026 — Partial IME capture:** Added the redacted IME health and proxy lines in Step 13. Label stays documentation-reviewed.

**19 September 2026:** Added a one-device home-lab notebook for a licensed freeware Win32 app and a broken detection rule. No tenant execution is claimed.

**10 September 2026 — Reviewed against documentation:** Clarified example status and used a verified Entra object ID to avoid ambiguous display-name matches. The code samples from that pass remain illustrative.
