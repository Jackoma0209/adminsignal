...
reviewNote: "Merged the standalone RSoP/gpresult tutorial into this canonical diagnosis page. The old tutorial URL now redirects here."
...
Restoration replaces settings and keeps the GPO GUID and links. Store backups before any significant policy change.
...
- **RSoP does not show all CSEs**: Software Installation and Folder Redirection are not included in RSoP output — use `gpresult /H` for those.
- **gpresult /R does not show the per-setting winning GPO**: use `gpresult /H` for the full per-setting breakdown.
- **gpresult reflects the current session**: it shows what applied at last logon or startup, not necessarily what would apply now if policies changed afterwards.
- **Non-Group Policy preferences**: PowerShell DSC, Intune, and registry edits outside GP are not visible in gpresult or RSoP. A setting may appear as "not configured" in GP while another management plane is setting it.
