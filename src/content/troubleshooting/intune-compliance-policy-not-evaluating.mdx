...
reviewNote: "Corrected custom compliance JSON semantics, quoted diagnostic areas, Windows platform applicability, event 814, aggregate Graph reporting and device identity selection."
...
- [Monitor device compliance](https://learn.microsoft.com/en-us/intune/device-security/compliance/monitor-policy)
...
mdmdiagnosticstool.exe -area "DeviceEnrollment;DeviceProvisioning" -cab "$logPath\diag.cab"
```
...
5. Confirm the policy platform matches the device OS. The Windows 10 and later platform includes Windows 11; inspect individual setting applicability and minimum/maximum OS constraints.
...
Check both Conversion Status and Protection Status. Fully Encrypted does not establish that protectors are active or a recovery key is backed up.
...
Expected result is True when Secure Boot is enabled on supported UEFI hardware. Supported virtual machines can also use Secure Boot; investigate their configuration before considering a scoped exception.
...
Document the grace period before escalating to the security team. A grace period delays the configured mark-noncompliant action. Inspect the aggregate device compliance state and sign-in result; another failed policy can still block access. See [actions for noncompliance](https://learn.microsoft.com/en-us/intune/intune-service/protect/actions-for-noncompliance).
...
$device = Get-MgDeviceManagementManagedDevice -ManagedDeviceId "<verified-intune-managed-device-id>" -ErrorAction Stop
...
Read aggregate compliance counts for one policy when you know its ID (this is not a device list):
...
| Event ID 814 | PolicyManager setting information; read the actual message, not an assumed conflict |
...
```powershell
powershell.exe -NoProfile -File "C:\Path\Detect-Compliance.ps1"
echo $LASTEXITCODE
```

A custom compliance discovery script returns a single compressed JSON object containing discovered values. Intune compares those values with the uploaded JSON rules. It does **not** use exit 1 for compliant and exit 0 for noncompliant; do not copy the Remediations exit-code contract here. Validate successful script execution and JSON keys, types and values against [Microsoft’s custom discovery-script requirements](https://learn.microsoft.com/en-us/intune/device-security/compliance/create-custom-script). Keep diagnostic chatter out of standard output.
...
- Custom compliance scripts are version-controlled and their JSON output is validated against the policy rule schema.
...
**10 September 2026 — Reviewed against documentation:** Corrected custom compliance JSON semantics, quoted diagnostic areas, Windows platform applicability, event 814, aggregate Graph reporting and device identity selection. Code and output remain illustrative, not execution evidence.
