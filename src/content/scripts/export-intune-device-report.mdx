...
Use this guide to build an authorised Intune inventory export for audit, troubleshooting, compliance review, licence reconciliation, and management reporting. The workflow is designed to minimise permissions and produce evidence without changing Intune configuration or device state.
...
```powershell
Export-IntuneDeviceReport -TenantId  '<tenant-id>' `
                          -ClientId  '<app-id>' `
                          -ClientSecret $secret `
...
The export workflow is read-only and makes no Intune, Entra ID, or device changes, so there is no script rollback action. Treat findings as input for a separate change-controlled process: device sync, compliance policy investigation, stale device review, licence cleanup, or helpdesk escalation.
...
| Version | Date | Notes |
|---|---|---|
| Guide v0.2 | Apr 27, 2026 | Added certificate-auth guidance, validation steps, and PII handling notes |
| Guide v0.1 | Feb 2025 | Initial Intune export implementation guide |
