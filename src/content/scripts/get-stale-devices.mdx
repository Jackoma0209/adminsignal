...
Use this guide to build a defensive device-hygiene report for authorised administrators. The goal is to identify stale records, compare state across management planes, and prepare a human-reviewed remediation queue. It is not intended for unauthorised discovery or automated removal of devices.
...
```powershell
$appId     = '<app-id>'
$appSecret = ConvertTo-SecureString '<app-secret>' -AsPlainText -Force
...
The reporting workflow makes no tenant or directory changes, so there is no script rollback action. Treat the remediation column as a queue for separate change-controlled work: verify ownership, check recent sign-ins or check-ins, confirm device retirement records, and only then delete or disable stale objects through your normal administrative process.
...
| Version | Date | Notes |
|---|---|---|
| Guide v0.2 | Apr 27, 2026 | Added Graph permission guidance, validation checks, and read-only safety notes |
| Guide v0.1 | Feb 2025 | Initial implementation guide |
