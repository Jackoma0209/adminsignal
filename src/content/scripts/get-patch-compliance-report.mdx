...
Use this guide to build an authorised patch-compliance evidence report for managed Windows fleets. The goal is to help administrators prioritise remediation, validate rollout rings, and explain patch exposure using approved WSUS, Windows Update for Business, and Graph data sources.
...
The reporting workflow does not approve, decline, install, or remove updates, so there is no script rollback action. Remediation should happen through your existing patch process: update ring assignment, WSUS approval workflow, Configuration Manager deployment, device sync, repair of Windows Update components, or helpdesk follow-up for stale/offline devices.
...
| Version | Date | Notes |
|---|---|---|
| Guide v0.2 | Apr 27, 2026 | Added WSUS/WUfB validation steps, throttling notes, and read-only safety framing |
| Guide v0.1 | Jan 2025 | Initial patch compliance implementation guide |
