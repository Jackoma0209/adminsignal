...
reviewNote: "Clarified emergency MFA and permanent active recovery roles, PIM boundaries, MAA prerequisites and safe validation."
...
- Keep phishing-resistant MFA available and verify both accounts at least every 90 days; custom CA exclusions do not bypass Microsoft mandatory MFA

Do not exclude regular admins "temporarily" and forget to remove the exclusion.

## Phase 4: Move Standing Privilege To PIM

For ordinary admin roles, use PIM with the applicable Entra ID P2 or Governance entitlement. Keep designated emergency Global Administrator accounts permanently active, as [Microsoft’s emergency-access design](https://learn.microsoft.com/en-us/entra/identity/role-based-access-control/security-emergency-access) requires, rather than making them depend on PIM activation.
...
After migration, verify effective permissions for the intended role. Direct Intune RBAC assignments and group-based access can still grant privileges independently of the Entra role being tested.
...
Before enabling protection, validate the manager, requestor and approver permissions and directly assigned approver security group against [Microsoft’s current MAA requirements](https://learn.microsoft.com/en-us/intune/fundamentals/role-based-access-control/multi-admin-approval). After a second administrator approves, the requestor must complete the request. Document turnaround and recovery; configure working approver RBAC before protecting role changes.
...
- Test approval with a reversible configuration change first; a wipe test requires a disposable device, verified recovery and explicit authorisation to erase it
...
**10 September 2026 — Reviewed against documentation:** Clarified emergency MFA and permanent active recovery roles, PIM boundaries, MAA prerequisites and safe validation. Examples are illustrative; no lab or tenant execution is claimed.
