...
If you are already managing BitLocker recovery prompts caused by a previous update in your environment, the [BitLocker recovery key Entra backup guide](/troubleshooting/bitlocker-recovery-key-not-backed-up-entra) covers escrow and recovery-key availability when PCR7 or Secure Boot-related prompts appear.
...
6. For the BitLocker PCR7 issue, confirm whether any additional devices in broader rings match the affected configuration. If recovery prompts appear after the May 2026 update, use the [BitLocker recovery key Entra backup guide](/troubleshooting/bitlocker-recovery-key-not-backed-up-entra).
...
- [BitLocker recovery key Entra backup guide](/troubleshooting/bitlocker-recovery-key-not-backed-up-entra) for escrow and recovery-key availability when PCR7 or Secure Boot changes trigger BitLocker prompts.
