...
## Home lab notebook (Ring 0, one quality update)

A home tenant will not look like Autopatch. Treat one VM as Ring 0. This revision does not claim the notebook was executed.

1. Record `Get-ComputerInfo` WindowsProductName, OsName, and OsBuildNumber before you assign policy.
2. Create a device group with only the lab VM.
3. Assign a Windows update ring: quality deferral 0 days, a deadline you can wait for, and **no** feature update policy that would move the VM to a new Windows release during the test.
4. Confirm Autopatch is not also targeting the VM. If it is, this notebook is invalid.
5. After the quality update installs and the VM has restarted if required, record the new build and the Intune update report status.
6. If the build does not move, collect the WUfB diagnosis evidence from [deferral not respected](/troubleshooting/wufb-deferral-not-respected) instead of widening the group.

<EvidenceCallout
  type="output"
  title="OS build before and after the Ring 0 quality update"
  capture="Get-ComputerInfo OsBuildNumber before and after, plus Intune Windows update status for that device"
  redact="Device name, UPN, and tenant"
  why="A deferral of zero days is not proof the update installed. The build number is."
/>

Do not set `labTested` until the before and after builds differ, or you have a named reason they did not.
...
**19 September 2026:** Added a one-device home-lab notebook for a Ring 0 quality update. No tenant execution is claimed.
