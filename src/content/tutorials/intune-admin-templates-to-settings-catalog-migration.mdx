...
This guide is written for Microsoft Intune tenants using Windows 10 and Windows 11 device management, with illustrative policy inventory and reporting queries for Microsoft Graph PowerShell. Their inclusion does not establish tenant execution.
...
Collection reads use the following helper in the same PowerShell session and follow [Microsoft Graph paging](https://learn.microsoft.com/en-us/graph/paging). This is an illustrative implementation, not a recorded tenant execution.
...
function Get-IntunePolicyAssignment {
    ...
    $Response = Get-GraphCollectionResponse -Uri $Uri
...
<ScreenshotSlot
  src="/images/evidence/intune-settings-catalog-create.png"
  alt="Microsoft Intune admin centre, Devices, Configuration, Create profile, Settings catalog, with the settings picker open and tenant identity redacted"
  caption="Intune Settings catalog profile creation, Configuration settings step, with the settings picker open. Tenant identity redacted."
/>
...
## Home lab notebook (one setting, two sources)

Create a conflict on purpose, then remove one source. Use Delivery Optimization **Download mode** because it exists in Administrative Templates and in Settings Catalog. See [Delivery Optimization reference](https://learn.microsoft.com/en-us/windows/deployment/do/waas-delivery-optimization-reference). This revision does not claim the notebook was executed.

1. Assign an Administrative Templates profile to the lab VM: Download mode HTTP only (0).
2. Assign a Settings Catalog profile to the same VM: Download mode LAN (1).
3. Sync. On the device, record per-setting status in Intune for both profiles. Expect a conflict, not a silent winner you guessed.
4. Record the effective value from the policy store or `Get-DeliveryOptimizationStatus` / DO registry so you know which source won, if either did.
5. Remove the Administrative Templates assignment. Sync again. Confirm the Settings Catalog value is now the only owner and the conflict has cleared.

<EvidenceCallout
  type="screenshot"
  title="Per-setting conflict then single owner"
  capture="Intune per-setting status showing conflict while both profiles are assigned, then Success after the Admin Template is removed, plus the effective Download mode value"
  redact="Device name, profile names if they identify a production tenant, and tenant name"
  why="A tidy Settings Catalog profile is not a migration. Proof is the conflict you created and the owner you left behind."
/>

Do not set `labTested` until the conflict and the cleared state both exist. Do not run this against a production ring.
...
**Verification clarification, 10 September 2026:** Environment descriptions are assumptions or documentation-review context. No complete lab/tenant execution is established by the material retained with this article; example commands and output are illustrative. Genuine individual captures do not verify the whole procedure.


**Correction — 19 September 2026:** Added a one-device home-lab notebook that creates a Delivery Optimization Download mode conflict. No tenant execution is claimed.

**Correction — 10 September 2026:** Added pagination to collection helper reads and clarified that examples are not execution evidence.
