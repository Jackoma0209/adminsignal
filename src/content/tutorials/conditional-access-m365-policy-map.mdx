...
reviewNote: "Corrected emergency MFA, compliant-device versus app-protection claims, risk-policy branching, compromise remediation and guest/workload-identity scope. This is a documentation review, not a tenant test."
...
**Illustrative environment (no execution evidence recorded)**: Entra ID P1 and P2 tenants, Microsoft 365 Business Premium and E3/E5 licensing, April 2026.

**Before deploying a new CA policy**: Validate at least two cloud-only emergency accounts with phishing-resistant credentials, permanent active emergency roles and exclusions from blocking Conditional Access. Exclusions do not bypass Microsoft mandatory MFA. See the [emergency access design](/tutorials/entra-break-glass-emergency-access-accounts).
...
**Exclusions**: The documented emergency access accounts. Certificate-based user authentication is not a reason for a blanket exemption. Service principals are a separate workload-identity scenario, not user accounts to exclude from this policy.
...
**What it does**: Blocks access from devices that are not Intune-compliant or hybrid-joined. A hybrid-join grant alone does not require Intune compliance. If the security requirement is a compliant device, configure that requirement explicitly. Devices meeting an OR alternative do not need to meet both controls.
...
**Platform scope**: MAM/app protection does not make an unmanaged device compliant. Mobile compliance requires supported enrolment and the appropriate platform/browser authentication path. Design a separate app-protection access path where needed. [Microsoft compliant-device policy guidance](https://learn.microsoft.com/en-us/entra/identity/conditional-access/policy-all-users-device-compliance).

## Policy 3: Block Legacy Authentication

**What it does**: Blocks legacy authentication flows that cannot satisfy MFA. SMTP AUTH, IMAP and POP can also use OAuth; the protocol name alone does not prove Basic authentication. [Microsoft legacy-authentication policy](https://learn.microsoft.com/en-us/entra/identity/conditional-access/policy-block-legacy-authentication).
...
Investigate the client and authentication method before changing exclusions. Prefer a supported OAuth migration. Any temporary exception needs a narrow scope, owner, expiry and compensating controls; do not broadly exempt a trusted IP range.
...
- Users: All users
- Cloud apps: All cloud apps
- For a simple pilot: Sign-in risk — Medium and High; Grant — Require multifactor authentication
- If the approved design uses a different action for High risk, create a separate policy for that condition; one policy cannot branch between two grant actions by risk level
...
**Remediation path**: When a user is blocked, an admin must navigate to **Entra admin centre → Identity Protection → Risky users**, locate the user, investigate the risk detections, and follow the documented remediation for the evidence found. Confirming compromise marks risk; it is not equivalent to disabling the account. Disable access, revoke sessions or require secure remediation as separate authorised actions when appropriate. Dismiss risk only after validating a false positive. [Microsoft risk remediation](https://learn.microsoft.com/en-us/entra/id-protection/howto-identity-protection-remediate-unblock).
...
5. Have **phishing-resistant MFA credentials**, such as FIDO2 keys or suitably configured certificate-based authentication, with dependencies distinct from normal administrators
...
- **Service principals and managed identities**: User CA policies do not protect service-principal calls. Workload-identity Conditional Access supports qualifying service principals, but does not cover managed identities. [Microsoft workload-identity scope](https://learn.microsoft.com/en-us/entra/identity/conditional-access/workload-identity).
- **Guest users**: Evaluate the resource tenant’s policy and cross-tenant trust settings. Home-tenant MFA/device claims can be trusted when configured; otherwise additional resource-tenant requirements may apply. [Microsoft cross-tenant access](https://learn.microsoft.com/en-us/entra/external-id/cross-tenant-access-overview).
...
**10 September 2026:** Corrected emergency MFA, compliant-device versus app-protection claims, risk-policy branching, compromise remediation and guest/workload-identity scope. This is a documentation review, not a tenant test.
