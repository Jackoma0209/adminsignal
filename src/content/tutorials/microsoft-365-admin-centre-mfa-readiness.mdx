...
reviewNote: "Marked the July 2026 postponement deadline as passed and linked the current escalation guidance."
...
```powershell
$RoleDefinitions = Get-GraphCollectionResponse -Uri "https://graph.microsoft.com/v1.0/roleManagement/directory/roleDefinitions?`$select=id,displayName,isBuiltIn,isPrivileged"
$RoleDefinitionById = @{}
...
$Assignments = Get-GraphCollectionResponse -Uri "https://graph.microsoft.com/v1.0/roleManagement/directory/roleAssignments?`$select=id,principalId,roleDefinitionId,directoryScopeId&`$top=999"
...
The helper follows every `@odata.nextLink` and stops on request errors. A partial or failed export is not a completed audit. Role-assignable groups still require a separate membership review.
...
$Eligibility = Get-GraphCollectionResponse -Uri $EligibilityUri
...
The 1 July 2026 Phase 2 postponement date is historical. If enforcement now blocks an essential workflow, follow the [current Microsoft support escalation guidance](https://learn.microsoft.com/en-us/entra/identity/authentication/concept-mandatory-multifactor-authentication) while migrating user-based automation to an appropriate workload identity. Do not assume an exception is available or granted.
...
**10 September 2026 — Reviewed against documentation:** Marked the July 2026 postponement deadline as passed and linked the current escalation guidance. Examples are illustrative; no lab or tenant execution is claimed.
