...
- [BitLocker recovery key Entra backup](/troubleshooting/bitlocker-recovery-key-not-backed-up-entra)
...
(Get-BitLockerVolume -MountPoint C:).KeyProtector |
    Select-Object KeyProtectorId, KeyProtectorType
```
...
--------------                      ----------------
{<recovery-password-protector-id>}    RecoveryPassword
```

This inventory deliberately selects protector IDs and types without printing recovery passwords. Match the current recovery-password protector ID to an authorised escrow record; the local listing alone does not prove backup. See [recovery-key validation](/troubleshooting/bitlocker-recovery-key-not-backed-up-entra).
...
- For devices with a known BitLocker recovery pattern, confirm escrow with [BitLocker recovery key Entra backup](/troubleshooting/bitlocker-recovery-key-not-backed-up-entra).
...
**Correction — 10 September 2026:** Selected BitLocker protector IDs without exposing recovery passwords; clarified escrow validation.
