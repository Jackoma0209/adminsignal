...
- **Recovery key not visible in Intune but visible in Entra**: This is expected behaviour. The Intune portal retrieves the key from Entra ID on demand. If Entra ID shows the key and Intune does not, it is a display delay — refresh the page.
