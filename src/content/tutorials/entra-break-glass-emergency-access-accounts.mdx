...
reviewNote: "Added a developer-tenant home-lab notebook. This revision does not claim that notebook was executed."
...
- Blocking Conditional Access controls, using documented exclusions. Security defaults do not offer per-user exclusions; a migration to Conditional Access must preserve equivalent protection and be separately planned
...
5. Assign **Global Administrator** as permanent active, not PIM eligible-only, to both accounts. Register the phishing-resistant credentials described below before accepting the design.
...
<EvidenceCallout
  type="screenshot"
  title="Emergency access accounts in Entra users"
  image="/images/evidence/entra-emergency-users.png"
  imageAlt="Redacted Microsoft Entra users list showing two cloud-only Emergency Access accounts with Global Administrator and a directory-synced user contrast row"
  capture="Microsoft Entra admin centre > Identity > Users, filtered to the two cloud-only emergency accounts, showing account enabled, assigned roles, and on-premises sync status"
  redact="Tenant name, any directory object IDs, and the real emergency UPNs if they identify the organisation"
  why="Break-glass only works if both accounts exist, are cloud-only, remain enabled, and hold Global Administrator. A federated or synced account will fail during the identity incident this design is meant to survive."
/>
...
Emergency accounts must satisfy Microsoft’s mandatory MFA requirements. A Conditional Access exclusion does not bypass platform-enforced MFA. Register phishing-resistant credentials with a different dependency from normal administrators. [Microsoft emergency access guidance](https://learn.microsoft.com/en-us/entra/identity/role-based-access-control/security-emergency-access).
...
There is no password-only bypass of mandatory Microsoft MFA in this design. Record credential dependencies and test the chosen method from a designated secure workstation.
...
1. Evaluate a new or replacement policy in report-only; keep existing production protections enabled while testing
...
3. From a designated secure workstation and clean browser profile, sign in to [https://entra.microsoft.com](https://entra.microsoft.com) as `breakglass-01`
4. Confirm you can open **Users**, **Conditional Access**, and role assignments
5. Sign out
6. Repeat for `breakglass-02`; validate both accounts at least every 90 days
...
## Home lab notebook (two cloud-only accounts in a developer tenant)

A Microsoft 365 developer tenant is enough. Do not create these accounts in an employer directory. This revision does not claim the notebook was executed.

1. Create `breakglass-01` and `breakglass-02` as cloud-only users with long random passwords stored in a password manager you own, not in a screenshot.
2. Assign Global Administrator as a **permanent active** role, not PIM-eligible-only. Register a phishing-resistant method if the tenant can; do not publish SMS as the finished design.
3. Exclude both accounts from every Conditional Access policy you actually use in the lab tenant. Confirm Security defaults are off or that the accounts still work under the tenant's current MFA requirement.
4. Create an alert for sign-in success on those UPNs.
5. Sign in once with `breakglass-01`, confirm the alert, sign out. Repeat for `breakglass-02` on a different day or session.
6. Record the 90-day test reminder. Delete the lab accounts only when you retire the developer tenant.

Microsoft's [emergency access guidance](https://learn.microsoft.com/en-us/entra/identity/role-based-access-control/security-emergency-access) is the design contract.

<EvidenceCallout
  type="screenshot"
  title="Two cloud-only emergency accounts and a sign-in alert"
  capture="Entra users showing both accounts enabled, cloud-only, Global Administrator, plus the monitoring alert from one successful sign-in"
  redact="UPNs if they identify a production tenant, object IDs, and any credential material"
  why="An unused break-glass account is a story. A monitored sign-in from both accounts is the notebook."
/>

Do not set `labTested` until both accounts have a monitored sign-in. Do not paste passwords into this repository.
...
**19 September 2026:** Added a developer-tenant home-lab notebook for two emergency accounts and a monitored sign-in. No tenant execution is claimed.

**10 September 2026:** Clarified mandatory MFA, security-defaults limitations, permanent active roles and validation of both emergency accounts. Removed advice to put existing production protection into report-only during redesign.
