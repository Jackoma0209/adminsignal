...
reviewNote: "Added a read-only home-lab notebook for app-only Graph. This revision does not claim that notebook was executed."
...
<ScreenshotSlot
  src="/images/evidence/entra-app-certificates-secrets.png"
  alt="Microsoft Entra admin centre, App registrations, Certificates and secrets, Client secrets tab, with secret values and IDs redacted"
  caption="Entra app registration Certificates and secrets, Client secrets tab. Secret values and IDs redacted. One secret is approaching expiry."
/>
...
For a fuller stale device workflow, use the [PowerShell hub](/powershell).
...
- A report export pattern such as [PowerShell topic hub](/powershell).
...
## Home lab notebook (app-only device count)

Register an app in the personal developer tenant only. Least privilege: `Device.Read.All` or `User.Read.All`, not `Directory.ReadWrite.All`. This revision does not claim the notebook was executed.

1. App registration, client secret or certificate, admin consent for the one read permission.
2. `Connect-MgGraph` with client credentials. Record `Get-MgContext` AuthType, TenantId (redact), and Scopes.
3. Run one count, not a dump:

```powershell
# Replace the three values from the lab app registration. Do not commit them.
Connect-MgGraph -ClientId $clientId -TenantId $tenantId -CertificateThumbprint $thumbprint -NoWelcome
Get-MgContext | Select-Object AuthType, Scopes
Get-MgDevice -CountVariable deviceCount -ConsistencyLevel eventual -Top 1 | Out-Null
Write-Output "DeviceCount=$deviceCount"
Disconnect-MgGraph | Out-Null
```

4. Rotate or delete the secret or certificate after the capture. Do not commit it.

<EvidenceCallout
  type="output"
  title="App-only Graph context and a single count"
  capture="Get-MgContext AuthType and Scopes, plus one DeviceCount or UserCount integer"
  redact="TenantId, ClientId, certificate thumbprint, and any object lists"
  why="A delegated Connect-MgGraph in the author's session is not app-only. The AuthType and a single count are the notebook."
/>

Do not set `labTested` until AuthType is app-only and the secret has been disposed. Do not paste a client secret into the article.
...
**Verification clarification, 19 September 2026:** Added a read-only app-only Graph notebook. No tenant execution is claimed.
