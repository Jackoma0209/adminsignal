...
reviewNote: "Retitled to match the collection-and-export scope actually published. Removed SQLite and multi-host database promises that were not implemented."
tags: ["PowerShell", "WMI", "Inventory", "Automation"]
---

## Why collect inventory without an agent

Enterprise RMM tools and Intune both provide software inventory. For a legacy environment, an air-gapped network, or an incident-response hour where you need to know what is installed on a machine you already administer, a PowerShell collection script is still useful.

This guide collects three *sources of evidence* and writes CSV or HTML:

1. **Registry uninstall keys** (primary source, including many non-MSI installs)
2. **Win32_Product WMI class** (MSI packages only; use sparingly)
3. **AppX / Store packages** (kept in a separate list so names do not collide)

It does **not** build a persistent inventory system. There is no SQLite schema, no upsert path, and no rolling multi-host database in this article. If you need that, use Intune discovered apps, Configuration Manager, or an approved RMM export.
...
Treat CSV and HTML as point-in-time exports. A persisted multi-host inventory database is out of scope for this page.
