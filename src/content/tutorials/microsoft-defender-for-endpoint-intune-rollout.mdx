...
2. Go to **Tenant administration > Connectors and tokens > Microsoft Defender for Endpoint**.
3. Check **Connection status**. Microsoft currently documents this as **Connected** when the service-to-service link is healthy.
...
<EvidenceCallout
  type="screenshot"
  title="Intune Defender for Endpoint connection status"
  image="/images/evidence/intune-defender-connector.png"
  imageAlt="Redacted Microsoft Intune connector page showing Microsoft Defender for Endpoint connection status as Connected, with the tenant identifier masked"
  capture="Intune admin centre > Tenant administration > Connectors and tokens > Microsoft Defender for Endpoint, showing Connection status and the Windows compliance policy evaluation toggle"
  redact="Tenant name, any device counts, and operator account identifiers"
  why="Onboarding policy and device-risk compliance both depend on this tenant connection. If the status is not enabled, later Sense-service checks will look like device problems when the connector is the actual gap."
/>
