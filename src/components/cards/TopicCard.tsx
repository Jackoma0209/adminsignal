...
export default function TopicCard({ topic }: TopicCardProps) {
  ...
  return (
    <article className="h-full">
      <Link
        href={`/${topic.slug}`}
        className="group flex h-full flex-col gap-3 rounded-xl border border-border bg-surface p-5 shadow-card transition-colors hover:border-border-strong hover:bg-surface-elevated/40"
      >
